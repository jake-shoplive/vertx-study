...
public class VertxMetricsMonitor {
    ...
    private int getLocalConsumerCount() {
        ...
        // 여기서는 추정치를 반환합니다
        return 4; // DeploymentVerticle 인스턴스 수
    }
    ...
    private void logThreadPoolInfo() {
        ...
        // Worker 풀 정보 (설정에서 확인)
        logger.info("- Worker pool size: 5 (from config)");
        logger.info("- Worker pool name: custom-pool");
    }
    
    private void logEventLoopInfo() {
        logger.info("EventLoop Information:");
        logger.info("- EventLoop instances: 4 (from config)");
